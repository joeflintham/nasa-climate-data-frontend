...
const linegraph = (data, el) => {
  const paddings = { top: 15, right: 15, bottom: 15, left: 40 };
  const margins = { top: 15, right: 15, bottom: 15, left: 15 };

  const bbox = el.getBoundingClientRect();
  
  const dims = { width: bbox.width - (margins.left + margins.right), height: 300 - (margins.top + margins.bottom) };
  ...
  const setupChart = (_el) => {
    ...
    const g_xAxis = svg.append('g')
        .attr('class', 'x-axis')
          .attr('transform', 'translate(0,' + (dims.height + paddings.bottom) + ')')
        .call(xAxis);
