...
var linegraph = function(data, el){
    ...
    var paddings = { top: 0, right: 0, bottom: 60, left: 60 };
    var margins = { top: 0, right: 15, bottom: 0, left: 0 };

    var bbox = el.getBoundingClientRect();
    var dims = {width: bbox.width - (margins.left + margins.right), height: bbox.height - (margins.top + margins.bottom)};
    ...
    var _setupChart = function(_el){
        ...
        var g_yAxis = svg.append("g").attr("class", "y-axis").attr("transform", "translate("+paddings.left+","+paddings.top+")")
            .call(yAxis);
        var g_xAxis = svg.append("g").attr("class", "x-axis").attr("transform", "translate(0,"+dims.height+")")
            .call(xAxis);
